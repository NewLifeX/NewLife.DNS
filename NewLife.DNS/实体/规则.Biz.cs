    ...
    public partial class Rule : Entity<Rule>
    {
        ...
        public static EntityList<Rule> FindAllByQueryTypeAndName(Int32 querytype, String name)
        {
            if (Meta.Count >= 1000)
            {
                ...
                var now = DateTime.Now;
                list.RemoveAll(e => e.IsExpired || !e.IsEnable);
                return list;
            }

            if ((QueryType)querytype != NewLife.DNS.QueryType.ANY)
            {
                return Meta.Cache.Entities.FindAll(e => e.QueryType == querytype && e.IsEnable && !e.IsExpired && e.IsMatch(name));
            }
            else
            {
                return Meta.Cache.Entities.FindAll(e => !e.IsExpired && e.IsEnable && e.IsMatch(name));
            }
        }
